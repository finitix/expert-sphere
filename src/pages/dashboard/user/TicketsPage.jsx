...
import { Ticket, MessageSquare, Star, Search, Plus, Eye, AlertCircle } from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search tickets..." value={search} onChange={(e) => setSearch(e.target.value)} className="gh-input pl-10 w-full"/>
          </div>
          <div className="flex gap-2">
            {["all", "pending", "in_progress", "completed"].map((f) => (<button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === f
        ...
        <div className="space-y-4">
          {filteredTickets.map((ticket, i) => (<motion.div key={ticket.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="gh-card hover:border-primary/30 transition-colors">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                ticket.status === "in_progress" ? "chip-primary" : "chip-warning"}`}>
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    {ticket.trainer ? (<div className="flex items-center gap-2">
                      ...
                      </div>) : (<span className="flex items-center gap-1 text-warning">
                        <AlertCircle className="w-4 h-4"/>
                        Waiting for trainer
                      </span>)}
                    <span>Created {ticket.created}</span>
                    {ticket.messages > 0 && (<span className="flex items-center gap-1">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    {ticket.status === "in_progress" && (<button className="gh-btn-primary px-3 py-2">
                        <MessageSquare className="w-4 h-4"/>
                      </button>)}
                  </div>
                </div>
