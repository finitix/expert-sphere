...
import { DollarSign, TrendingUp, ArrowUpRight, ArrowDownLeft, Download, CreditCard } from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        <div className="gh-card">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              {["week", "month", "year"].map((p) => (<button key={p} onClick={() => setPeriod(p)} className={`px-3 py-1 rounded text-sm ${period === p ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
          ...
          <div className="flex items-end justify-between gap-2 h-48">
            {monthlyData.map((data, i) => (<div key={data.month} className="flex-1 flex flex-col items-center gap-2">
                <motion.div initial={{ height: 0 }} animate={{ height: `${(data.amount / maxAmount) * 100}%` }} transition={{ duration: 0.8, delay: i * 0.1 }} className="w-full bg-primary/80 rounded-t-md min-h-[20px] relative group cursor-pointer hover:bg-primary">
        ...
        <div className="gh-card">
          ...
          <div className="space-y-3">
            {earnings.map((earning, i) => (<motion.div key={earning.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-border">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${earning.status === "pending" ? "bg-warning/20" : "bg-success/20"}`}>
                    {earning.status === "pending" ? (<ArrowDownLeft className="w-5 h-5 text-warning"/>) : (<ArrowUpRight className="w-5 h-5 text-success"/>)}
                  </div>
                  <div>
