        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="mb-10 p-4 rounded-xl bg-muted/50 border border-border">
            ...
            <div className="flex flex-wrap gap-2">
              {[...sections, ...additionalSections.map(s => ({ ...s, icon: FileText }))].map((s) => (<a key={s.title} href={`#section-${s.title.split(".")[0].replace(/\D/g, "")}`} className="text-xs px-2.5 py-1 rounded-md bg-background border border-border text-muted-foreground hover:text-foreground hover:border-primary/50 transition-colors">
                  {s.title}
                </a>))}
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            return (<section key={section.title} id={`section-${num}`} className="p-6 rounded-xl bg-card border border-border scroll-mt-24">
