...
import { Shield, CheckCircle2, XCircle, Award, ExternalLink, Clock } from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-3">
            ...
            {pendingVerifications.map((applicant, i) => (<motion.div key={applicant.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} onClick={() => setSelectedId(applicant.id)} className={`gh-card cursor-pointer transition-all ${selectedId === applicant.id ? "border-primary bg-primary/5" : "hover:border-muted-foreground/30"}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className={`text-xs font-medium ${applicant.score >= 90 ? "text-success" : applicant.score >= 80 ? "text-primary" : "text-warning"}`}>
          ...
          <div className="lg:col-span-2">
            {selectedApplicant ? (<motion.div key={selectedApplicant.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="gh-card">
                <div className="flex items-start justify-between mb-6">
                  ...
                  <div className={`px-4 py-2 rounded-lg text-center ${selectedApplicant.score >= 90 ? "bg-success/20 text-success" :
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedApplicant.expertise.map(skill => (<span key={skill} className="chip chip-primary">{skill}</span>))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedApplicant.certifications.map(cert => (<span key={cert} className="chip">{cert}</span>))}
                    </div>
                  </div>
              ...
              </motion.div>) : (<div className="gh-card flex items-center justify-center h-full min-h-[400px]">
