...
import { Search, Eye, AlertCircle, CheckCircle2, Clock, XCircle } from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2">
            {["all", "pending", "in_progress", "completed", "disputed"].map((f) => (<button key={f} onClick={() => setStatusFilter(f)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${statusFilter === f ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"}`}>
        ...
        <div className="space-y-3">
          {filteredTickets.map((ticket, i) => (<motion.div key={ticket.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="gh-card flex items-center justify-between">
