...
const Login = () => {
    ...
    return (<PageTransition>
      <div className="min-h-screen bg-background flex">
        ...
        ].map((item, i) => (<motion.div key={item} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.1 }} className="flex items-center gap-3">
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="w-full max-w-md">
            ...
            <div className="flex rounded-lg bg-muted p-1 mb-6">
              {["user", "trainer"].map((r) => (<button key={r} onClick={() => setRole(r)} className={cn("flex-1 py-2 px-4 rounded-md text-sm font-medium transition-all", role === r
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={isLoading} className="w-full btn-primary py-2.5 disabled:opacity-50 disabled:cursor-not-allowed">
                {isLoading ? (<span className="flex items-center justify-center gap-2">
                    ...
                    Signing in...
                  </span>) : (`Sign in as ${role === "user" ? "Student" : "Teacher"}`)}
              </button>
            </form>
