...
import { BadgeCheck, Search, Star, Eye, Ban, MessageSquare } from "lucide-react";
...
const AdminTrainersPage = () => {
    ...
    const filteredTrainers = trainers.filter(trainer => {
        if (search && !trainer.name.toLowerCase().includes(search.toLowerCase()))
            return false;
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search trainers..." value={search} onChange={(e) => setSearch(e.target.value)} className="gh-input pl-10 w-full"/>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {filteredTrainers.map((trainer, i) => (<motion.div key={trainer.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="gh-card">
