...
import { Wallet, CreditCard, ArrowUpRight, ArrowDownLeft, Plus, DollarSign, Clock } from "lucide-react";
...
const PaymentsPage = () => {
    ...
    return (<DashboardLayout>
      <div className="space-y-6">
        ...
        ].map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === tab.id
        ...
        {activeTab === "overview" && (<div className="grid md:grid-cols-2 gap-6">
            {/* Recent Transactions */}
            <div className="gh-card">
              <h3 className="font-semibold text-foreground mb-4">Recent Transactions</h3>
              <div className="space-y-3">
                {transactions.slice(0, 4).map((tx) => (<div key={tx.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
        ...
        {activeTab === "transactions" && (<div className="gh-card">
            <div className="space-y-3">
              {transactions.map((tx, i) => (<motion.div key={tx.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center justify-between p-4 rounded-lg bg-muted/30 border border-border">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center ${tx.amount > 0 ? "bg-success/20" : "bg-muted"}`}>
                      {tx.amount > 0 ? (<ArrowDownLeft className="w-6 h-6 text-success"/>) : (<ArrowUpRight className="w-6 h-6 text-muted-foreground"/>)}
                    </div>
                    <div>
        ...
        {activeTab === "methods" && (<div className="gh-card max-w-xl">
