...
import { Star, Award, MapPin, Calendar, Globe, Github, Linkedin, Camera, Save, Plus, X } from "lucide-react";
...
const ProfilePage = () => {
    ...
    return (<DashboardLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Profile Header Card */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="gh-card">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="gh-card">
          ...
          <textarea defaultValue="10+ years building scalable web applications. Ex-Google engineer specializing in React and cloud architecture. I love helping developers solve complex problems and level up their skills." rows={4} className="gh-input w-full resize-none"/>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="gh-card">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {skills.map((skill) => (<span key={skill} className="chip chip-primary flex items-center gap-1">
                {skill}
          ...
          <div className="flex gap-2">
            <input type="text" value={newSkill} onChange={(e) => setNewSkill(e.target.value)} onKeyPress={(e) => e.key === "Enter" && addSkill()} placeholder="Add a skill..." className="gh-input flex-1"/>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="gh-card">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input type="number" defaultValue={75} className="gh-input w-32 pl-8"/>
            </div>
        ...
        {/* Certifications */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="gh-card">
          <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
            <Award className="w-5 h-5 text-primary"/>
            Certifications
          </h3>
          <div className="space-y-3">
            {["AWS Solutions Architect", "Google Cloud Professional", "MongoDB Certified"].map((cert) => (<div key={cert} className="flex items-center justify-between p-3 rounded-lg bg-muted/50 border border-border">
        ...
        {/* Social Links */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="gh-card">
