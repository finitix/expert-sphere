...
function TrainerCard({ trainer, isCenter }) {
    ...
    return (<div ref={cardRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className="transition-all duration-500" style={{
      ...
      <GlassCard className="w-72 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-4">
          {trainer.skills.map((skill) => (<span key={skill} className="chip text-xs">
              {skill}
            </span>))}
        </div>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="relative">
          <div className="flex items-center justify-center gap-6 py-8">
            {getVisibleTrainers().map(({ trainer, isCenter }, index) => (<div key={`${trainer.id}-${index}`} className="hidden md:block" style={{ display: index === 2 || window.innerWidth >= 768 ? 'block' : 'none' }}>
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            <button onClick={() => navigate('prev')} className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-colors">
              <ChevronLeft className="w-5 h-5"/>
            </button>
            
            {/* Dots */}
            <div className="flex gap-2">
              {trainers.map((_, index) => (<button key={index} onClick={() => {
                setIsAutoPlaying(false);
            ...
            <button onClick={() => navigate('next')} className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-colors">
