...
import { Star, CheckCircle2, MapPin, Calendar, Clock, MessageSquare, Award, Github, Linkedin, Twitter, Globe } from "lucide-react";
...
const TrainerProfile = () => {
    ...
    const trainer = trainerData[username] || trainerData.sarahc;
                  ...
                  <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex-shrink-0 -mt-20 md:-mt-24 relative z-10">
                      <div className="relative">
                        <img src={trainer.avatar} alt={trainer.name} className="w-24 h-24 md:w-32 md:h-32 rounded-full object-cover border-4 border-card"/>
                        {trainer.online && (<span className="absolute bottom-2 right-2 w-5 h-5 bg-success rounded-full border-4 border-card"/>)}
                      </div>
                    </div>
                  ...
                  <FadeInView>
                    <div className="gh-card">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {trainer.skills.map((skill) => (<span key={skill} className="chip-primary">
                            {skill}
                          </span>))}
                      </div>
                    </div>
                  ...
                  <FadeInView>
                    <div className="gh-card">
                      ...
                      <div className="space-y-3">
                        {trainer.recentSolves.map((solve, i) => (<div key={i} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                            <div>
                            ...
                            <div className="flex items-center gap-0.5">
                              {Array.from({ length: solve.rating }).map((_, j) => (<Star key={j} className="w-3 h-3 fill-warning text-warning"/>))}
                            </div>
                          </div>))}
                      </div>
                    </div>
                  ...
                  <FadeInView>
                    <div className="gh-card">
                      ...
                      <div className="space-y-4">
                        {trainer.recentReviews.map((review, i) => (<div key={i} className="pb-4 border-b border-border last:border-0 last:pb-0">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <div className="flex items-center gap-1">
                                {Array.from({ length: review.rating }).map((_, j) => (<Star key={j} className="w-3 h-3 fill-warning text-warning"/>))}
                              </div>
                            </div>
                ...
                <div className="space-y-6">
                  ...
                  <FadeInView>
                    <div className="gh-card">
                      ...
                      <ul className="space-y-2">
                        {trainer.certifications.map((cert, i) => (<li key={i} className="text-sm text-muted-foreground flex items-center gap-2">
                  ...
                  <FadeInView>
                    <div className="gh-card">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {trainer.languages.map((lang) => (<span key={lang} className="chip">
                            {lang}
                          </span>))}
                      </div>
                    </div>
                  ...
                  <FadeInView>
                    <div className="gh-card">
                      ...
                      <div className="space-y-2">
                        {trainer.socials.github && (<a href={`https://github.com/${trainer.socials.github}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                            <Github className="w-4 h-4"/>
                            github.com/{trainer.socials.github}
                          </a>)}
                        {trainer.socials.linkedin && (<a href={`https://linkedin.com/in/${trainer.socials.linkedin}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                            <Linkedin className="w-4 h-4"/>
                            linkedin.com/in/{trainer.socials.linkedin}
                          </a>)}
                        {trainer.socials.twitter && (<a href={`https://twitter.com/${trainer.socials.twitter}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                            <Twitter className="w-4 h-4"/>
                            @{trainer.socials.twitter}
                          </a>)}
                        {trainer.socials.website && (<a href={`https://${trainer.socials.website}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
