...
import { Ticket, Clock, CheckCircle2, Wallet, ArrowRight, MessageSquare, Star, Plus, Eye } from "lucide-react";
    ...
    return (<DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, i) => (<motion.div key={stat.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="stats-card">
              <div className="flex items-center justify-between mb-2">
                ...
                {stat.trend && (<span className={`text-xs font-medium ${stat.trend.startsWith("+") ? "text-success" : "text-muted-foreground"}`}>
                    {stat.trend}
                  </span>)}
              </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "overview" && (<motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="gh-card">
                ...
                <div className="space-y-3">
                  {myTickets.slice(0, 3).map((ticket) => (<div key={ticket.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-border hover:border-primary/30 transition-colors cursor-pointer">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-mono text-muted-foreground">{ticket.id}</span>
                          <span className={`chip text-xs ${ticket.status === "in_progress" ? "chip-primary" : "chip-warning"}`}>
                            {ticket.status.replace("_", " ")}
                          </span>
                        </div>
                        <p className="font-medium text-foreground text-sm truncate">{ticket.title}</p>
                        {ticket.trainer && (<p className="text-xs text-muted-foreground mt-1">
                            Working with {ticket.trainer.name}
                          </p>)}
                      </div>
                      <div className="text-right flex-shrink-0 ml-4">
                        <p className="font-mono font-semibold text-primary">{ticket.budget}</p>
                        {ticket.messages > 0 && (<span className="text-xs text-muted-foreground flex items-center justify-end gap-1">
              ...
              <div className="gh-card">
                ...
                <div className="space-y-1">
                  {recentActivity.map((item, i) => (<div key={i} className="flex items-start gap-3 py-3 border-b border-border/50 last:border-0">
          ...
          {/* Tickets Tab */}
          {activeTab === "tickets" && (<motion.div key="tickets" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
              {myTickets.map((ticket) => (<div key={ticket.id} className="gh-card hover:border-primary/30 transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-3">
                        {ticket.trainer ? (<div className="flex items-center gap-2">
                          ...
                          </div>) : (<span className="text-sm text-muted-foreground">Waiting for trainer...</span>)}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {ticket.status === "in_progress" && (<button className="gh-btn-primary px-4 py-2 text-sm">
          ...
          {/* History Tab */}
          {activeTab === "history" && (<motion.div key="history" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="gh-card">
              <h3 className="font-semibold text-foreground mb-4">Completed Tickets</h3>
              <div className="space-y-3">
                {completedTickets.map((ticket) => (<div key={ticket.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/30 border border-border">
                    <div className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-1 text-warning text-xs justify-end">
                        {[...Array(ticket.rating)].map((_, i) => (<Star key={i} className="w-3 h-3 fill-current"/>))}
                      </div>
                    </div>
                  </div>))}
              </div>
