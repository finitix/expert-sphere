...
import { Zap, LayoutDashboard, Ticket, MessageSquare, Wallet, Settings, Users, BadgeCheck, TrendingUp, Shield, LogOut, Bell, Plus } from "lucide-react";
    ...
    return (<div className="min-h-screen bg-background flex">
      ...
      <aside className="w-64 border-r border-border bg-sidebar fixed left-0 top-0 bottom-0 flex flex-col">
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-center gap-3">
            <div className={cn("w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold text-white", roleColors[user?.role || "user"])}>
              {user?.avatar}
            </div>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (<Link key={item.href} to={item.href} className={cn("sidebar-link", isActive && "active")}>
      ...
      <main className="flex-1 ml-64">
        ...
        <header className="h-16 border-b border-border bg-background/95 backdrop-blur-sm sticky top-0 z-40 flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            {user?.role === "user" && (<Link to="/create-ticket" className="btn-primary flex items-center gap-2">
        ...
        {/* Page Content */}
        <motion.div key={location.pathname} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="p-6">
          {children}
        </motion.div>
      </main>
    </div>);
