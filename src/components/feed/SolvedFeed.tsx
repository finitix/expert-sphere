...
const solvedTickets: SolvedTicket[] = [
  {
    id: "1",
...
interface SolvedFeedProps {
  limit?: number;
}

export function SolvedFeed({ limit }: SolvedFeedProps) {
    ...
    <div className="border border-border rounded-lg overflow-hidden divide-y divide-border">
      {displayTickets.map((ticket, index) => (
        <motion.article
          ...
          className="feed-item bg-card hover:bg-card-hover"
        >
          {/* Header */}
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <Link 
                  to={`/trainer/${ticket.trainer.username}`}
                  className="font-semibold text-foreground hover:underline"
                >
                  {ticket.trainer.name}
                </Link>
                {ticket.trainer.verified && (
              ...
              <div className="mt-2 flex items-center gap-1">
                ...
                {Array.from({ length: 5 - ticket.rating }).map((_, i) => (
